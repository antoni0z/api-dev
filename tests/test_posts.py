...
@pytest.mark.parametrize("title, content, published",[
    ...
    ("Tallest skyscrapers","Burj Khalifa, welcome to Dubai habibi", True),
])
...
def test_authorized_user_delete_nonexistent_post(authorized_client,test_user,test_posts):
    res = authorized_client.delete("/posts/800")
    assert res.status_code == 404
...
def test_authorized_user_update_nonexistent_post(authorized_client,test_user,test_posts):
    data = {
        "title": "updated title",
        "content": "updated content",
        "id": test_posts[0].id
    }
    res = authorized_client.put("/posts/800",json = data)
    assert res.status_code == 404